...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_proj.NANOGPT_SCALE_INIT = torch.tensor(1)
...
# run the training loop (if multi GPU avail)
...
ddp = int(os.environ.get('RANK', -1)) != -1 # is this run a DDP run?
if ddp:
    # use of DDP requires (based on docs from some while back) CUDA, so we set appropriate device according to rank
...
def get_lr(it):
    ...
    # 2: if it > lr_decay_iters, returning min lr
    if it > max_steps:
        return min_lr
...
for step in range(max_steps):
    ...
    # eval our validation loss from time to time
    ...
    if (step % 250 == 0 or last_step) and (not use_compile): # execute every 250th iter, or at the last step, ONLY if not using compile
        num_correct_norm = 0
        num_total = 0
        for i, example in enumerate(iterate_examples("val")):
            # only process examples where i % dpp_world_size == ddp_rank
    ...
    for micro_step in range(grad_accum_steps):
        ...
        x, y = x.to(device), y.to(device)
        with torch.autocast(device_type=device, dtype=torch.bfloat16): # use mixed precision for better performance
            logits, loss = model(x, y)
        # we have to scale the loss to account for the grad accum
        # because the gradients add on each succesive backward()
        # addition of gradients corresponds to a SUM in the objective
        # but instead of a SUM we want MEAN. Scale the loss here so it comes out right
